...
public class ChatAdaptor extends RecyclerView.Adapter {
    ...
    public int getItemViewType(int position) {
        // sender type
        if (dataHolder.get(position).getPublicUid() != null && dataHolder.get(position).getPublicUid().equals(myPreference.getPublicUid())) {
            if (dataHolder.get(position).getType() != null && dataHolder.get(position).getType().equals("text")) {
                return SENDER_VIEW_TYPE;
            } else if (dataHolder.get(position).getType() != null && dataHolder.get(position).getType().equals("note")) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        String noteTitle, noteBody, decryptedNoteTitle, decryptedNoteBody, decryptedUsername, decryptedPassword;
        String[] title1;
        String userName, password, websiteName, websiteLink, title, currentDate;
        try {
            if (holder.getClass() == SenderViewHolder.class) {
                ...
                try {
                    ...
                    userName = temp.getAddDataLogin();
                    password = temp.getAddDataPassword();
                    ...
                    ((SenderPasswordViewHolder) holder).tvLogin.setText(decryptedUsername);
                ...
                try {
                    ...
                    ((ReceiverPasswordViewHolder) holder).tvLogin.setText(decryptedUsername);
