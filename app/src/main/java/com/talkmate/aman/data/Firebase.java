...
import com.talkmate.aman.home.addpassword.WebsiteHelperClass;
import com.talkmate.aman.iAES;
import com.talkmate.aman.messages.ChatModelClass;
import com.talkmate.aman.messages.UserListModelClass;
import com.talkmate.aman.notes.addnote.NoteHelperClass;
import com.talkmate.aman.signin_login.UserHelperClass;
...
public class Firebase implements iFirebaseDAO {
    ...
    @Override
    public void loadChatRoomID(String receiverPublicUid, Firebase.iChatRoomID iChatRoomID) {
        System.out.println("receiverPublicUid: " + receiverPublicUid + "myPreference.getPublicUid(): " + myPreference.getPublicUid());
        FirebaseDatabase.getInstance().getReference("messageUserList")
                .child(myPreference.getPublicUid()).child("userPersonalChatList").child(receiverPublicUid).child("chatRoomID").addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            chatRoomID = dataSnapshot.getValue(String.class);
                            System.out.println("chatRoomID: " + chatRoomID);
                            iChatRoomID.onReturnChatRoomID(chatRoomID);
                        }
                    }
    ...
    @Override
    public void setTyping(String receiverPublicUid) {
        System.out.println("receiverPublicUid: " + receiverPublicUid + "myPreference.getPublicUid(): " + myPreference.getPublicUid());
        FirebaseDatabase.getInstance().getReference("messageUserList")
                .child(myPreference.getPublicUid()).child("userPersonalChatList")
                .child(receiverPublicUid).child("isTyping").setValue(true);
    }

    @Override
    public void isTyping(String receiverPublicUid, iTyping iTyping) {
        FirebaseDatabase.getInstance().getReference("messageUserList")
                .child(myPreference.getPublicUid()).child("userPersonalChatList")
                .child(receiverPublicUid).child("isTyping").addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            boolean temp = dataSnapshot.getValue(Boolean.class);
                            iTyping.isTypingOnSuccess(temp);
                        }
                    }
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            ...
                            for (DataSnapshot ds : dataSnapshot.getChildren()) {
                                ...
                                if (chatModel != null) {
                                    System.out.println("chatModel.getType(): " + chatModel.getType());
                                    if (chatModel.getType() != null && chatModel.getType() == ("note")) {
                                        // Print raw values from the DataSnapshot
                                        System.out.println("Raw type value: " + ds.child("type").getValue());
                                        System.out.println("Raw message value: " + ds.child("message").getValue());
                                    }
                                }
    ...
    public interface iChatRoomID {
        void onReturnChatRoomID(String chatRoomID);
    }

    public interface iTyping {
        void isTypingOnSuccess(boolean isTyping);
    }
}
